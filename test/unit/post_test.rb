  ...
  context "Moderation:" do
    context "A deleted post" do
      ...
      context "that is undeleted" do
        ...
        context "by the approver" do
          setup do
            @post.update_attribute(:approver_id, @moderator.id)
          end
        ...
        context "by the uploader" do
          setup do
            @post.update_attribute(:uploader_id, @moderator.id)
          end
    ...
    context "An unapproved post" do
      ...
      context "that was previously approved by person X" do
        setup do
          @post = FactoryBot.create(:post, :approver_id => @moderator.id)
          @post.flag!("bad")
        end

        should "not allow person X to reapprove that post" do
          approval = @post.approve!(@moderator)
        ...
        should "allow person Y to approve the post" do
          @post.approve!(FactoryBot.create(:moderator_user, :name => "yyy"))
          assert(@post.valid?)
        end
      end
        ...
        context "for a pool" do
          ...
          context "when the post is pool locked" do
            setup do
              ...
              @post.add_pool!(@pool1)
              create(:post_lock, post: @post, creator: @builder, pools_lock: true)
            end
