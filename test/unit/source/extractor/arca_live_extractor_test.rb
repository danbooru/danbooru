...
module Source::Tests::Extractor
  class ArcaLiveExtractorTest < ActiveSupport::ExtractorTestCase
    ...
    context "An Arca.live image URL with a referer" do
      setup do
        skip "This test is flaky because the image URL expires after a certain amount of time"
      end

      strategy_should_work(
        "https://ac.namu.la/20221225sac2/e06dcf8edd29c597240898a6752c74dbdd0680fc932cfd0ecc898795f1db34b5.jpg?expires=1772289273&key=FLrFbwBGOWR-ndIcp4ti7A",
    ...
    context "An Arca.live page URL with an animated .gif" do
      strategy_should_work(
        ...
        updated_at: nil,
        tags: [],
        dtext_artist_commentary_title: "스포)부들부들 떠는 아리스 그림 점프 추가해봤어",
    ...
    context "An Arca.live page URL with a fake .mp4 GIF" do
      strategy_should_work(
        "https://arca.live/b/zenlesszonezero/159393370",
        image_urls: [%r{https://ac\.namu\.la/20260112sac/22f43e42ef9f124d1a917325b284dd5343bca9faf886607a7d81ad083b7db306\.gif\?expires=\d+&key=[\w_-]+&type=orig}],
        media_files: [{ file_size: 390_778 }],
        page_url: "https://arca.live/b/zenlesszonezero/159393370",
        profile_url: "https://arca.live/u/@나는야잡초맨",
        profile_urls: %w[https://arca.live/u/@나는야잡초맨],
        display_name: nil,
        username: "나는야잡초맨",
        published_at: nil,
        updated_at: nil,
        tags: [],
        dtext_artist_commentary_title: "모든 걸 뺏긴 복복이",
        dtext_artist_commentary_desc: <<~EOS.chomp,
          [video]

          https://www.youtube.com/embed/E2BCCqPFvjs?start=92

          그 집이 내 집이였어야 해~

          "X(트위터)":[https://x.com/imaweedman01/status/2010632843542593786]
        EOS
      )
    end
    ...
    context "An Arca.live page URL with an animated emoticon" do
      strategy_should_work(
        ...
        # media_files: [
        #   { file_size: 12_312_152 },
        #   { file_size: 12_608_871 },
        # ],
        ...
        updated_at: nil,
        tags: [],
        dtext_artist_commentary_title: "최고로 이쁜 산짤써맛이에용",
        dtext_artist_commentary_desc: <<~EOS.chomp,
          울이쁜이들과함께 같이 다과회를 즐겨요
        EOS
      )
    end
  end
