  ...
  context "The media assets controller" do
    ...
    context "show action" do
      ...
      should "not show embedding dtext links in non-visible models" do
        @mod_topic = create(:forum_topic, min_level: User::Levels::MODERATOR, original_post: build(:forum_post, body: "!asset ##{@upload2.media_assets.first.id}"))

        get media_asset_path(@upload2.media_assets.first)
        assert_not_select "a[href='/forum_posts/#{@mod_topic.original_post.id}']"

        get_auth media_asset_path(@upload2.media_assets.first), create(:moderator_user)
        assert_select "a[href='/forum_posts/#{@mod_topic.original_post.id}']"
      end
