  ...
  class Preprocessor
    ...
    def start!
      if Utils.is_downloadable?(source)
        ...
        if Upload.where(source: [source, canonical_source], status: ["preprocessing", "preprocessed"]).exists?
          raise ActiveRecord::RecordNotUnique.new("A preprocessed upload with source #{source} already exists")
        end
