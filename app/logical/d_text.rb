...
class DText
  ...
  def self.format_text(text, data: nil, inline: false, disable_mentions: false, base_url: nil, domain: Danbooru.config.hostname)
    return nil if text.nil?
    data = preprocess([text]) if data.nil?
    text = parse_embedded_tag_request(text)
    html = DText.parse(text, inline: inline, disable_mentions: disable_mentions, base_url: base_url, domain: domain)
