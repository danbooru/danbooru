...
class Source::Extractor
  class Twitter < Source::Extractor
    ...
    def parsed_get(path, cache: 1.minute, **params)
      ...
      response = http.cache(cache, key: cache_key, if: ->(response) { response.status != 429 && !response.status.server_error? }).headers(headers).get("https://x.com#{path}", params: params)
