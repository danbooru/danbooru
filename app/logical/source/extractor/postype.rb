...
class Source::Extractor::Postype < Source::Extractor
  ...
  def dtext_artist_commentary_desc
    return "" if artist_commentary_desc.to_s.parse_html.tap { it.css("section, script").remove }.text.blank?
    ...
    DText.from_html(artist_commentary_desc, base_url: "https://www.postype.com") do |element|
      ...
      in "a" if element.children.any? { it.name == "img" } && element.next&.name == "a" && element.next.children.any? { it.name == "img" }
        element.after("<br>")
  ...
  memoize def post_content_from_html
    script = page&.css("body script").to_a.find { |script| script.text =~ /self.__next_f.push\(.*post-content*/ }
    return nil unless script.present?
    script_text = script.text[/self.__next_f.push\(\[1,(".*")\]\)$/, 1]
    begin
      JSON.parse(script_text)
