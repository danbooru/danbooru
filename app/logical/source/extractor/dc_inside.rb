...
class Source::Extractor::DcInside < Source::Extractor
  ...
  def image_urls_from_commentary
    page&.css(".writing_view_box").css("img, video").css(":not(.written_dccon)").to_a.filter_map do |el|
      src = if el.name == "video"
        el.attr("data-src")
      else
        el.attr("src")
      end
      ...
      if el.attr('onclick').to_s.match %r{\Ajavascript:imgPop\('([^']*)'}
        src = $1
      end
