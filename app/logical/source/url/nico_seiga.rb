...
module Source
  class URL::NicoSeiga < Source::URL
    ...
    def parse
      ...
      # https://embed.nicovideo.jp/watch/sm36465441?autoplay=1
      in _, "nicovideo.jp", "watch", /^(..)\d+$/ => video_id
        @video_id = video_id
      ...
      # https://sp.seiga.nicovideo.jp/image/source?id=3521156
      # https://sp.seiga.nicovideo.jp/image/redirect?id=3521156 (doesn't work)
      ...
      in _, "nicovideo.jp", "user", /^\d+$/ => user_id, *rest
        @user_id = user_id
    ...
    def image_url?
      host.in?(%w[lohas.nicoseiga.jp dcdn.cdn.nimg.jp drm.cdn.nicomanga.jp]) || (host.match?(/seiga.nicovideo.jp/) && path.starts_with?("/image/"))
    end
