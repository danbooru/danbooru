...
class Ban < ApplicationRecord
  PERMITTED_DURATIONS = [1.day, 3.days, 1.week, 1.month, 3.months, 6.months, 1.year, 100.years]
  ...
  validates :duration, inclusion: { in: PERMITTED_DURATIONS, message: "%{value} is not a valid ban duration" }, if: :duration_changed?
  ...
  def create_feedback
    user.feedback.create!(
      creator: banner,
      category: "negative",
      body: "Banned #{ApplicationController.helpers.humanized_duration(duration)}: #{reason}",
  ...
  def create_dmail
    Dmail.create_automated(
      ...
      body: "You have been banned #{forever? ? "forever" : "for #{ApplicationController.helpers.humanized_duration(duration)}"}: #{reason}"
    )
  end

  def create_ban_mod_action
    ModAction.log(
      %{banned <@#{user_name}> #{ApplicationController.helpers.humanized_duration(duration)}: #{reason}}, :user_ban,
      subject: user,
      user: banner
    )
  end
