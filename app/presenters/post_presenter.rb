class PostPresenter < Presenter
  ...
  def self.preview(post, options = {})
    ...
    if options[:show_cropped] && post.has_cropped? && !CurrentUser.user.disable_cropped_thumbnails?
      srcset = ["#{post.crop_file_url} 150w", "#{post.preview_file_url} 150w"].join(", ")
      sizes = ["(max-width: 480px) #{Danbooru.config.small_image_width}px", "(min-width: 480px) #{Danbooru.config.small_image_width}px"].join(", ")
    else
      srcset = nil
      sizes = nil
    end
